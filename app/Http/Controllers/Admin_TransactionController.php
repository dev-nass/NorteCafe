...
class Admin_TransactionController
{
    ...
    public function table()
    {
        ...
        $transactions = $db
            ->query("SELECT transactions.transaction_id, CONCAT(users.first_name, ' ', users.last_name) as customer_detail, CONCAT(employees.first_name, ' ', employees.last_name) as employee_detail, transactions.created_at, transactions.payment_method, transactions.amount_due, transactions.amount_tendered, transactions.sukli, transactions.status
                FROM transactions
                LEFT JOIN users ON transactions.user_id = users.user_id
                LEFT JOIN employees ON transactions.transaction_id = employees.employee_id")
            ->get();
    ...
    public function show()
    {
        ...
        $transactions =
            $db->query("SELECT transactions.*, 
                ...
                employees.employee_id, CONCAT(employees.first_name, ' ', employees.last_name) as employee_name, employees.contact_number,
                ...
                LEFT JOIN employees ON employees.employee_id = transactions.rider_id
    ...
    /**
     * Use for updating status of a transaction
     */
    public function update()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($updatedStatus) {
                redirect("transaction-pending-show?id={$_POST['transaction-id']}");
            }
        }
    }
}
